...
<body>
  <main>
    ...
    <section class="grid-2">
      ...
      <div class="panel">
        <h2>CSV読み込み確認</h2>
        <p class="small">入力後のCSVをここへ読み込むと、件数と先頭行を確認できます。実績値が入っていないCSVは未判定のままです。</p>
  ...
  <script>
    ...
    fileInput.addEventListener('change', async () => {
      ...
      const headHtml = headers.slice(0, 8).map((h) => \`<th>\${escHtml(h)}</th>\`).join('');
      const bodyHtml = data.slice(0, 5).map((items) => \`<tr>\${items.slice(0, 8).map((v) => \`<td>\${escHtml(v)}</td>\`).join('')}</tr>\`).join('');
      preview.innerHTML = \`
        <p><b>\${escHtml(file.name)}</b> を読み込みました。データ行: \${data.length}件、列: \${headers.length}列。</p>

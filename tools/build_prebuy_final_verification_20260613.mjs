...
function statusClass(status) {
  if (/停止|不可|未入力|未確認|悪化/.test(status)) return "bad";
  if (/注意|条件|保留|手動/.test(status)) return "warn";
  if (/通過|可|完了/.test(status)) return "ok";
  return "";
}

function insertCard(file, anchorHref, cardHtml, markerText) {
  const fullPath = path.join(ROOT, file);
  let text = fs.readFileSync(fullPath, "utf8");
  if (text.includes(markerText)) return;

...
<main>
  ...
  <div class="cards">
    ...
    <div class="card"><b>一次候補</b><span>${scored.filter((row) => row.status === "一次候補").length}</span><p>信頼度と警戒条件を通過</p></div>
  ...
  <h2>3. 使い方</h2>
  <p>この画面では、まず「一次候補」を次の詳細調査へ進めます。「監視」は点数が高くても急騰・急落・高ボラティリティを確認します。「データ補完」はPER/PBR/ROE、決算成長率、受注残、事業別売上比率を入れるまで購入候補にはしません。</p>

...
def add_page_6_plan(c: canvas.Canvas):
    ...
    intro = (
        "想定リターンは、正規10社の期待値と投入比率を組み合わせて確認します。"
        "初回から全額を入れず、55%・75%・85%の各段階で追加条件を確認します。"
    )
    draw_wrapped(c, intro, 46, 506, 746, 12, 18, INK, max_lines=2)
    ...
    rows = [
        ...
        ("本命計画", "75%", f"{target_ev:.1f}%", "初回40% + 5営業日後15.5% + 20営業日後19.5%", "5日・20日反応、決算、金利、為替、悪材料を通過", "条件未達なら現金待機。別銘柄で無理に補わない"),
        ("強気標準", "85%", f"{bull_ev:.1f}%", "EV上位5社へ各2%追加。240万円なら各48,000円", "TOPIX/S&Pが崩れず、出来高を伴う上昇、テーマ継続", "85%超の追加は別条件で確認"),
    ]
...
def add_page_6_stage_math(c: canvas.Canvas):
    ...
    intro = (
        "想定リターンは、全額を一度に入れず、条件通過ごとに投入率を上げて確認します。"
        "初回40%、5営業日後55.5%、20営業日・決算確認後75%、強い地合いで85%が目安です。"
    )
    ...
    rows = [
        ...
        ("強気標準", "本命75%到達後", "2,040,000円", "85.0%", "25.60%", "+25%を狙う場合の中心ライン。追加余力は別条件で判定"),
    ]
    ...
    msg = "第1段階だけでは約12.86%で、採用下限に届きません。第2段階で約16.85%となり採用下限を超え、第3段階で約22.13%となり本命計画に入ります。85%は+25%が見える強気標準ラインです。85%を超える追加は、さらに条件がそろった場合だけ検討します。"
    ...
    msg2 = "第2段階・第3段階の条件を満たさない場合、投入率だけを上げるために別銘柄へ振り替えません。期待値が作れない時は、現金待機とします。"
...
def add_page_6_stage_gate(c: canvas.Canvas):
    ...
    rows = [
        ...
        ("55.5% -> 75%", "20営業日・決算後", "20日反応、決算、出来高、金利・為替が許容範囲", "46.8万円追加", "条件外の振替買いはしない", "22.13%"),
        ("75% -> 85%", "強気標準確認後", "指数が崩れず、EV上位5社が出来高を伴って上昇", "24万円追加", "75%のまま維持", "25.60%"),
    ...
    msg = "第2段階を通過して初めて採用下限を超えます。本命+20%前後は第3段階まで進んだ場合だけです。85%は+25%が見える強気標準ラインで、85%を超える追加は押し目・確認後・最終判断の条件を満たす場合だけ検討します。"
...
def add_page_6_bull_execution(c: canvas.Canvas):
    title(c, "25%を狙うための実行条件", "85%は上限ではなく強気標準ライン")
    intro = (
        "+25%前後は、75%まで通過した後に追加条件を満たす場合だけ狙います。"
        "追加対象はEV寄与が大きく、出来高・指数差・材料が崩れていない5銘柄に限定します。"
    )
    draw_wrapped(c, intro, 46, 522, 746, 12.5, 18, INK, max_lines=2)

    formula = "計算: 75%投入時22.13% + 追加5社のEV寄与 = 25.60%。追加額は各48,000円、合計240,000円です。"
    draw_wrapped(c, formula, 60, 488, 720, 10.5, 13, NAVY, max_lines=1)
    ...
    draw_wrapped(c, "根拠CSV: regular10_bull_25_execution_20260625.csv。85%到達までの追加5銘柄、追加額、期待値寄与、通過条件、停止条件を記録しています。", 60, 44, 710, 9.2, 11.5, NAVY, max_lines=2)
    footer(c, 9)
    c.showPage()


def add_page_6_reserve15(c: canvas.Canvas):
    title(c, "85%超の追加判断", "最大100%まで進める場合の条件")
    intro = "85%投入は、強い地合いで+25%前後を狙うための中心ラインです。追加資金は条件を満たす場合だけ使います。"
    draw_wrapped(c, intro, 46, 512, 746, 12.5, 18, INK, max_lines=1)

    rounded_box(c, 46, 442, 746, 54, colors.HexColor("#FFF8E7"), colors.HexColor("#E3B15C"), 8)
    formula = "前提: 240万円運用。85%投入=204万円、未投入資金36万円。追加する場合は12万円ずつ3回に分けます。EV上位5社へ均等追加する場合、5%追加ごとのモデル寄与は約+1.73ptです。"
    draw_wrapped(c, formula, 62, 475, 710, 10.3, 13.5, NAVY, max_lines=2)

    rows = [
        ("押し目追加", "5%", "120,000円", "90%", "約27.33%", "有力銘柄が-3〜-7%下落。ただし決算・テーマ根拠は崩れていない", "下落が根拠崩れなら使わない"),
        ("確認後追加", "5%", "120,000円", "95%", "約29.07%", "20営業日・決算・イベント通過後、対TOPIX劣後なし", "指数劣後、悪材料、出来高急増下落なら待つ"),
        ("最終攻勢", "5%", "120,000円", "100%", "約30.80%", "運用全体がTOPIX比+3pt以上、主力5社の根拠が継続", "業種集中・過熱・急騰追い買いなら使わない"),
    ]
    draw_reserve15_table(c, rows, 36, 264)

    rounded_box(c, 46, 154, 746, 74, colors.HexColor("#F7FBFF"), colors.HexColor("#C9D9EA"), 8)
    c.setFillColor(BLUE)
    c.setFont(FONT_BOLD, 12)
    c.drawString(62, 206, "運用上の答え")
    msg = "85%は+25%前後を狙う強気標準ラインです。そこから先の追加は、勝ち筋が確認できた時だけ行います。条件がそろえば100%まで使い、条件が崩れれば現金を残します。"
    draw_wrapped(c, msg, 62, 188, 704, 10.3, 13, INK, max_lines=3)

    rounded_box(c, 46, 74, 746, 58, colors.HexColor("#FFF7F7"), colors.HexColor("#E4A0A0"), 8)
    c.setFillColor(RED)
    c.setFont(FONT_BOLD, 11)
    c.drawString(62, 112, "禁止ルール")
    draw_wrapped(c, "85%超の追加資金は、条件確認後に使う追加余力です。根拠が崩れた時、指数に劣後した時、急騰日の追い買いになる時は使いません。", 62, 95, 704, 9.8, 12.5, INK, max_lines=2)

    draw_wrapped(c, "根拠CSV: regular10_reserve15_deployment_20260625.csv。85%超の追加条件、追加金額、停止条件、期待値目安を記録しています。", 60, 44, 710, 9.2, 11.5, NAVY, max_lines=2)
    footer(c, 10)
    c.showPage()


def draw_reserve15_table(c: canvas.Canvas, rows, x0, y0):
    headers = ["枠", "追加率", "追加額", "累計", "目安", "使う条件", "止める条件"]
    widths = [76, 58, 78, 60, 78, 240, 210]
    row_h = 52
    header_h = 25
    top = y0 + row_h * len(rows)
    c.setFillColor(LIGHT_BLUE)
    ...
    for idx, row in enumerate(rows):
        ...
        for j, (val, w) in enumerate(zip(row, widths)):
            c.setStrokeColor(LINE)
            c.rect(xx, y, w, row_h, fill=0, stroke=1)
            color = GREEN if j == 4 else (RED if j == 6 else INK)
            draw_wrapped(c, val, xx + 4, y + row_h - 12, w - 8, 7.6, 10.2, color, max_lines=4)
            xx += w
...
def add_page_7_initial_buy(c: canvas.Canvas):
    ...
    intro = (
        ...
        "条件に合わない銘柄の金額は現金待機とし、ほかの銘柄へ無理に振り替えません。"
    )
    draw_wrapped(c, intro, 46, 500, 746, 12, 18, INK)
...
def add_page_8_additional_buy(c: canvas.Canvas):
    title(c, "追加買付の具体案", "40%から75%、強気標準では85%へ")
    intro = (
...
def add_page_9_pre_order_gate(c: canvas.Canvas):
    ...
    cards = [
        ...
        ("現金待機", "条件不一致分", "他銘柄へ無理に振り替えず、追加判定日まで残す。"),
    ]
...
def add_page_6(c: canvas.Canvas):
    ...
    c.setFont(FONT, 10)
    c.setFillColor(NAVY)
    c.drawString(chart_x, chart_y + chart_h + 58, "240万円で見た場合の資産推移目安")
    ...
    note = "グラフは資産管理の目安です。指数想定+10%は比較対象であり、目標ではありません。採用下限+15%を下回るなら追加しません。本命+20%は75%投入、強気標準+25%は85%投入。85%超の追加は別条件で判断します。"
...
<body>
  ...
  <main>
    <section>
      ...
      <a class="btn" href="regular10_reserve15_deployment_20260625.csv">追加余力運用CSV</a>
    ...
    <section>
      ...
      <div class="cards">
        ...
        <div class="card"><b>強気標準</b><strong>+25%</strong><span>85%で到達目安</span></div>
        <div class="card"><b>予備15%</b><strong>5%×3</strong><span>条件付きで追加</span></div>
